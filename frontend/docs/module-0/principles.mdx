...
- `\mathcal{T}: \mathcal{S} × \mathcal{A} × \mathcal{S} \rightarrow [0, 1]`: Transition probability function (how actions affect state)
- `\mathcal{R}: \mathcal{S} × \mathcal{A} \rightarrow \mathbb{R}`: Reward function (task objectives)
- `\mathcal{O}`: Set of possible observations (sensor measurements)
- `\mathcal{Z}: \mathcal{S} × \mathcal{O} \rightarrow [0, 1]`: Observation probability function (sensor noise model)
...
```
t_compute + t_communication < T_control = 1 / f_control
```

where f_control = 100 Hz implies T_control = 10 ms.
...
1. **Sense**: IMU measures angular velocity `ω` and linear acceleration `a` at 1000 Hz
2. **Estimate**: Complementary filter fuses IMU with joint encoder data to estimate body orientation `θ` and position `p`
3. **Control**: If `|θ - θ_desired| > \epsilon`, adjust foot placement or generate corrective torques
...
1. **Localization**: Each robot must estimate its 2D pose `(x, y, θ)` using wheel odometry and overhead fiducial markers, fusing measurements with a Kalman filter to handle wheel slip and sensor noise.
...
```
p_ZMP = Σ[m_i × (p_i - g/||g||) × (p̈_i - g)] / ||Σ[m_i × (p̈_i - g)]||_z
```

where m_i are link masses, p_i are link positions, p̈_i are link accelerations, and g is gravity.
...
```
F_i = K_p (x_i^d - x_i) + K_d (\dot{x}_i^d - \dot{x}_i)
```

where `F_i` is the desired force from arm `i`, `x_i` is the current end-effector pose, `x_i^d` is the desired pose, and `K_p, K_d` are stiffness and damping matrices.

*Equation 1.4: Impedance control for compliant manipulation*

By setting low stiffness `K_p`, the arms become compliant and can adapt to unexpected contact forces—critical when the object's exact geometry or pose is uncertain [7].
