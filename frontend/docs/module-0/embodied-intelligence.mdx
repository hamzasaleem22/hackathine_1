...
1. **Data Collection**: Execute random actions, record $(S_t, A_t, S_{t+1})$ tuples
2. **Model Learning**: Train neural network $\hat{f}_\theta$ to predict $S_{t+1}$ from $(S_t, A_t)$
...
$$
\mathbf{x}_{\text{desired}} = \mathbf{x}_{\text{nominal}} + \mathbf{C}_f (\mathbf{f}_{\text{measured}} - \mathbf{f}_{\text{desired}})
$$

where $\mathbf{C}_f$ is compliance matrix (how much position changes per unit force).
...
3. **A robot arm has joint angle sensors but no force sensors. Design a passive mechanism to safely interact with humans during collaborative assembly.**
   ...
   **Structure**:
   ...
   - Spring deflection $x$ measured indirectly: $x = \theta_{\text{motor}} - \theta_{\text{link}}$

   **Operation**:
   - Spring compliance means unexpected contact forces cause spring compression (not rigid arm motion)
   - Maximum force physically limited by spring: $F_{\text{max}} = k \cdot x_{\text{max}}$ where $k$ is spring stiffness
   - Choose $k$ such that $F_{\text{max}} < 50$ N (safe for human contact per ISO/TS 15066)
...
4. **How do sensorimotor contingencies enable object recognition without explicit 3D models?**
   ...
   **Example—Cube Recognition**:
   ...
   A robot that learns these contingencies (e.g., via forward model $\hat{S}_{t+1} = f(S_t, A_t)$) can distinguish cubes from spheres:
