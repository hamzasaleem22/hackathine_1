...
4. Distance computed: d = ct/2 where c = 3 × 10<sup>8</sup> m/s (speed of light)
...
where R is rotation matrix (3×3) and t is translation vector (3×1).
...
- Depth: `z = (c φ)/(4\pi f)` where `φ` is phase shift, `f` is modulation frequency
...
| **Cost** | Low (`50-`500) |
| **Resolution** | Dense (640×480 to 1920×1080 pixels) |
...
**3-Axis Gyroscope**: Measures angular velocity `ω = (ω_x, ω_y, ω_z)` in rad/s around each axis.

**3-Axis Accelerometer**: Measures linear acceleration `a = (a_x, a_y, a_z)` in m/s². Includes both:
...
**Goal**: Estimate robot orientation `q` (as quaternion) from gyroscope and accelerometer readings.

**Complementary Filter** (simple, real-time):

1. **Gyroscope Integration** (high-frequency, no drift correction):
   $$
   q_gyro(t+\Delta t) = q(t) \otimes \exp\left((\Delta t)/(2) ω\right)
   $$

2. **Accelerometer Tilt** (low-frequency, corrects drift):
   - Assumes `a ≈ g` (gravity) when robot is stationary
   - Compute orientation from gravity direction

3. **Fusion**:
   $$
   q(t+\Delta t) = \alpha q_gyro + (1-\alpha) q_accel
   $$
...
- State: `x = [q, ω, b_ω]` (orientation, angular velocity, gyro bias)
- Prediction: Integrate gyroscope
- Update: Correct with accelerometer + magnetometer

#### 4.3.3 Challenges

**Gyroscope Drift**: Integration of `ω` accumulates errors over time → orientation drifts (1-10°/minute for consumer IMUs)

**Accelerometer Noise**: During dynamic motion, `a \neq g` → cannot use accelerometer for tilt correction
...
1. **Grasping Stability**: Detect slip → increase grip force
2. **Texture Recognition**: Differentiate materials (metal vs. plastic vs. fabric)
3. **Safety**: Detect unexpected contact with humans → stop motion
...
1. **Segmentation**: Identify table plane, remove it → remaining points are objects
...
def segment_objects_on_table(depth_image, camera_intrinsics):
    ...
    # Remove table plane → objects remain
...
# Usage
depth = cv2.imread('depth_image.png', cv2.IMREAD_ANYDEPTH) / 1000.0  # mm → m
...
while True:
    ...
    if status == "critical":
        print("🚨 BALANCE CRITICAL - Emergency stop")
...
- **Accel Correction**: When robot is near-stationary, accelerometer measures gravity → correct gyro drift
...
**State**: `x = [x, y, z, q_w, q_x, q_y, q_z, v_x, v_y, v_z]` (position, orientation quaternion, velocity)
...
where `u_k` is IMU measurement (accel, gyro) and `w_k \sim \mathcal{N}(0, Q)` is process noise.
...
where `z_k` is pose from vision and `v_k \sim \mathcal{N}(0, R)` is measurement noise.
...
sequenceDiagram
    ...
    loop Every 33 ms (30 Hz)
        Vision->>EKF: Pose estimate (x, y, θ)
...
- ✅ **Tactile sensors enable safe manipulation**: Force/pressure sensors detect contact, slip, and texture. Essential for dexterous grasping and human-robot collaboration. Technologies range from simple FSRs (`1-`10) to high-resolution optical sensors ($1,000+).
...
- ✅ **Sensor selection depends on application requirements**: Outdoor navigation → LIDAR (long range, lighting-independent); Indoor manipulation → RGB-D (dense, semantic-rich); Dynamic control → IMU (high-rate, proprioceptive). Cost, computational requirements, and environmental conditions constrain choices.
...
2. **Explain why IMU-only pose estimation drifts over time. How does fusing IMU with visual odometry solve this?**
   ...
   **Gyroscope Integration**: Orientation is computed by integrating angular velocity `ω`:
   $$
   q(t) = q(0) \otimes \int_0^t ω(τ) dτ
   $$
   ...
   - Integration of noise accumulates as random walk: `\sigma_θ(t) \propto \sqrt{t}`

   **Accelerometer Drift**: Position requires double integration of acceleration:
   $$
   p(t) = p(0) + \int_0^t \int_0^τ a(τ') dτ' dτ
   $$
...
3. **Design a sensor suite for a humanoid robot working in a warehouse: picking boxes, navigating aisles, avoiding forklifts. What sensors would you choose and why?**
   ...
   **Recommended Sensor Suite**:
   ...
   2. **RGB-D Camera (wrist-mounted on each arm)**:
      ...
      - **Specs**: 640×480 depth, 30 Hz, 0.5-3 m range (RealSense D435)
      - **Cost**: `200 × 2 = `400
   ...
   5. **Tactile Sensors (gripper fingers)**:
      ...
      - **Cost**: `100 × 2 = `200
      - **Why**: Detect if box is slipping → adjust grip force
   ...
   **Sensor Fusion**: IMU + LIDAR odometry → EKF pose estimate for navigation. RGB-D + F/T sensors → grasp success verification.
   </details>
...
4. **An RGB-D camera computes depth via stereo matching. What happens to depth accuracy as distance increases? Why?**
   ...
   **Uncertainty Analysis**: If disparity has uncertainty `\sigma_d` (typically ±0.5 to ±1 pixel due to matching errors), depth uncertainty is:
   $$
   \sigma_z = (\partial z)/(\partial d) \sigma_d = (fb)/(d^2) \sigma_d = (z^2)/(fb) \sigma_d
   $$
   ...
   **Numerical Example** (RealSense D435: `f=` 525 px, `b=` 50 mm):
   - At 1 m: `\sigma_z = (1^2)/(525 × 0.05) × 0.5 ≈ 0.019` m = 1.9 cm
   - At 3 m: `\sigma_z = (3^2)/(525 × 0.05) × 0.5 ≈ 0.17` m = 17 cm
   - At 5 m: `\sigma_z = (5^2)/(525 × 0.05) × 0.5 ≈ 0.48` m = 48 cm

   **Why This Happens**:
   - Disparity `d` decreases with distance (farther objects → smaller pixel difference)
...
5. **You implement a complementary filter for IMU orientation with `\alpha = 0.98` (98% gyro, 2% accel). What happens if you increase `\alpha` to 0.999? What if you decrease to 0.90?**
   ...
   **When to use**: High-dynamic motions (acrobatics, fast head tracking) where accelerometer readings are mostly dynamic acceleration (not gravity) → can't use for tilt correction anyway
   ...
   **Effect**:
   ...
   - **Cons**: Sluggish response to actual motion; during dynamic maneuvers (jumping, fast turns), accelerometer measures `a \neq g` → filter incorrectly interprets as tilt

   **When to use**: Slow-moving or stationary robots where gyro drift is primary concern and dynamic accelerations are minimal

   **Failure Mode**: During a jump, accelerometer measures large upward acceleration → filter thinks robot is tilted back → outputs incorrect orientation during flight phase
...
- **[← Previous: Humanoid Robotics Landscape](./humanoid-landscape)**
- **[Module Overview](.)** | **[Module 0 Assessment](./assessment)**
- **[Next Module: ROS 2 & Robotics Middleware →](../module-1/)**
