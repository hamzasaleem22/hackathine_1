...
4. Distance computed: $d = \frac{ct}{2}$ where $c = 3 \times 10^8$ m/s (speed of light)
...
where $\mathbf{R}$ is rotation matrix (3×3) and $\mathbf{t}$ is translation vector (3×1).
...
**Accelerometer Noise**: During dynamic motion, $\mathbf{a} \neq \mathbf{g}$ → cannot use accelerometer for tilt correction
