...
- **Energy Efficiency**: Cost of Transport (COT) = $\frac{E}{mgd}$ where $E$ is energy consumed, $m$ is mass, $g$ is gravity, $d$ is distance
...
$$
\min_{\tau} \|\mathbf{J}^\top \tau - \mathbf{F}_{\text{desired}}\|^2 + \lambda \|\tau\|^2
$$

subject to: $\tau_{\min} \leq \tau \leq \tau_{\max}$, friction cone constraints
...
$$
\pi_\theta(\mathbf{a}_t | \mathbf{o}_t) = \text{Transformer}(\text{OccupancyNet}(\text{images}_t))
$$
